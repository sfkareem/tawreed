...
def test_excel_operations():
    ...
    try:
        ...
        # Formula check
        amount_cell_formula = ws_mas[
            "F2"
        ].value  # Row 2 in Master corresponds to the first data row
        assert amount_cell_formula == "=IFERROR(D2*E2, 0)", (
            f"Expected formula, got: {amount_cell_formula}"
        )
